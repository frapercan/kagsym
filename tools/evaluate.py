...
def main():
    ...
    p.add_argument("--opponent", default=E.V48.name, help="public agent, or 'passive' (solitaire)")
    ...
    opp = E.passive() if a.opponent == "passive" else E.public(a.opponent, a.cap)
    offset = None if a.no_offset else "checkpoint"
    world = {"hours": a.hours, "days": a.days}
    results = []
    for path in a.checkpoints:
        spec = E.PolicySpec(os.path.abspath(path), offset=offset)
        todo = [(spec, opp, s, seat, world) for s in seeds for seat in seats]
        eps = [ep for _, ep in E.run_tasks(todo, procs=a.procs, progress=True)]
