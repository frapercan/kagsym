...
| `search/best`, `search/median`, `search/played`, `search/seconds` (per generation), `result/search_seeds`, `result/unseen_seeds`, `best/<param>` | a portfolio search (`tools/plan_blocks.py`, run name `portfolio/<id>`): the fitness is money against the search rival (a recorded tape or passive) on the search seeds | `result/unseen_seeds` is the honest number of the search; the live consolidation is the yardstick. |
| `candidate/wins`, `candidate/money`, `candidate/rival_money` (per candidate), `best/wins`, `best/money`, `best/money_se`, `best/rival_money` | a live consolidation (`tools/consolidate_live.py`, run name `consolidate/<id>`): the top ten of a search against a live public rival on 20 seeds, seat 0 | `best/wins` is the criterion (the ladder scores wins). |
| `block/money`, `block/escaped`, `block/unplaced_days`, `block/feed_bought`, `block/idle`, `block/move`, `sold/<product>` | the expansion ladder (`tools/expansion_ladder.py`, run name `block/<A|B|C>`): a farm block played solo under a literal plan, 5 seeds | executor changes are judged here first, on all three blocks. |
| `panel/<plan>_wins_<rival>_seat<k>` | a wins panel against several public agents | |
